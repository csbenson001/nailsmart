...
class AppUsersController extends Controller
{
    ...
    public function login(Request $request)
    {
       ...
       /* if ($user) {
            return response()->json(['msg' => 'user found', 'data' => null, 'success' => true], 200);
        }
        else{
            return response()->json(['msg' => 'user not found', 'data' => null, 'success' => true], 200); 
        }*/
        //if ($user && Hash::check($request->password, $user->password)) { //benson - remove password requirement temporarily
        if ($user) {
            if ($user['verified'] != 1) {
